...
if($_SERVER['REQUEST_METHOD'] == 'POST' && array_key_exists('approved', $input)){
    ...
    // get log
    $sql_get = 'SELECT selfie_timestamp as time_stamp, log_type, id, image_path, is_selfie, latlng, approval_status FROM tbl_logs WHERE id=:id';
    ...
    try {
        ...
        // get
        $sql3=$conn->prepare($sql_get);
        $sql3->bindParam(':id', $id, PDO::PARAM_INT);
        $sql3->execute();
        $result_sql3 = $sql3->fetch(PDO::FETCH_ASSOC);
        echo json_encode($result_sql3);
